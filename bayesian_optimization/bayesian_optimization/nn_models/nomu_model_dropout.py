...
from bayesian_optimization.utils.utils import read_list_of_float_from_config, config_int_or_none, read_list_of_int_from_config
...
class NOMUModelDropout(NNModel):
    ...
    @classmethod
    def create_from_context(cls, context: Context) -> 'NOMUModelDropout':
    ...
    def fit(self, x: np.array, y: np.array, *args, **kwargs) -> Model:
        ...
        history = self.model.fit(x=gen, callbacks=callbacks, verbose=0, *args, **kwargs)
    ...
    @classmethod
    def read_from_config(cls, config) -> 'NOMUModelDropout':
        ...
        return NOMUModelDropout(
            ...
            dropout_prob=config.as_float("dropout_prob"),
        )

    def create_copy(self, dropout_prob=None, regularizer_factor=1.0):
        """creating a copy of a Network model
        :param base_seed: seed to copy
        :param regularizer_factor: regularizer factor to use
        :param dropout_prob: dropout_prob to use
        :return:
        """
        return NOMUModelDropout(
            layers=[self.main_layers, self.side_layers],
            loss=None,
            lowerbound_x_aug=self.lowerbound_x_aug,
            upperbound_x_aug=self.upperbound_x_aug,
            n_aug=self.n_aug,
            c_aug=self.c_aug,
            mu_sqr=self.mu_sqr,
            mu_abs=self.mu_abs,
