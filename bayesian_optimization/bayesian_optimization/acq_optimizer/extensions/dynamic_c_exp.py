...
from bayesian_optimization.utils.utils import config_float_or_none
# Type hints
from typing import *
if TYPE_CHECKING:
...
class DynamicCExponential(OptimizerCallback):
    ...
    def __init__(
            ...
            end_eps: float = 0.001,
            start_eps: Union[float, None] = None,
    ...
    def _in_eps(self, new_sample_x: np.array) -> bool:
        ...
        step_eps = self.start_eps * ((self.end_eps/self.start_eps)**(n_step/self.n_steps))
    ...
    def run(
        ...
        """run the actual call back, checks if potential new sample is within epsilon distance to samples.
        ...
        :return: potentially updated next sample, next samples mu estimates,
            next samples sigma estimates and next samples acq. value
        """
    ...
    @classmethod
    def read_from_config(cls, config: 'ConfigObj') -> 'DynamicCExponential':
        """reads the configuration of the exponential DynamicC Callback from the given configfile and returns the
        accordingly created exponential DynamicC Callback
        :param config: config parser instance
        :return: exponential DynamicC Callback
        """
        return cls(
            ...
            start_eps=config_float_or_none(config["start_eps"]),
