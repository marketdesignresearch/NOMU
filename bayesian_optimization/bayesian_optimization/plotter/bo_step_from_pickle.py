...
class BOStep2DPlot(Plot2D):
    ...
    def __init__(self, figsize, title, out_path, data, context):
    ...
    def draw_true_f(self, ax=1, linestyle="-", color="black", label="True Function", in_legend=True, **kwargs):
        ...
        y_true = [y[0] for y in self.context.callback(self.test_x)]
    ...
    def draw_uncertainty_bounds(self, ax=1, color="C0", alpha=0.3, label="Uncertainty Bounds", in_legend=True, **kwargs):
        self._add_twinx_if_needed(ax)
        h = self.axis[ax-1].fill_between(self.test_x[:,0], self.mu[:,0] - self.sig[:,0], self.mu[:,0] + self.sig[:,0], color=color, alpha=alpha, **kwargs)
        if in_legend:
    ...
    def draw_sample_vert_line(self, ax=1):
        for x in self.samples_x:
            self.axis[ax-1].axvline(x=x, linewidth=0.3, color="gray", alpha=1.0)
        return self

    def set_axis_ylabel(self, ax, label):
        self.axis[ax-1].set_ylabel(label)
        return self
