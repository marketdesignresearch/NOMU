...
class MeanWidthScaled(AbstractDecorator):
    ...
    def __init__(
        ...
        self.gridpoints = get_grid_points_multidimensional(self.lower_bound,self.upper_bound, self.n_test_points)
        self.c = None
    ...
    def evaluate(self, mu: np.ndarray, sigma: np.ndarray, incumbent: float) -> np.ndarray:
        ...
        if hasattr(self.context, "inspector") and self.context.inspector is not None:
            self.context.inspector.dump_data(self.INSP_C, self.c)
        return self.acq_to_decorate.evaluate(mu, self.c*sigma, incumbent)

    def sigma_corrected(self, sigma: np.ndarray):
        return self.c * sigma

    def evaluate_inspector_mu_sig(self, mu: np.ndarray, sigma: np.ndarray, context, step):
        if self.c is None or not self.once_only:
            self._calculate_scale()
        return self.acq_to_decorate.evaluate_inspector_mu_sig(mu, self.c*sigma, context, step)


    def evaluate_inspector_acq(self, mu: np.ndarray, sigma: np.ndarray, context, step):
    ...
    def _get_mean_uncertainty_width(self) -> float:
        ...
        """
        mean, sigma = self.context.estimator.regress(test_x=self.gridpoints)
