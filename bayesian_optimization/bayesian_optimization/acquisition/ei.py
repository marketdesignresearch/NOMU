...
class ExpectedImprovement(AcquisitionFunction):
    ...
    def _single_evaluate(self, mu: np.array, sigma: np.array, incumbent: np.array) -> np.array:
        ...
        if sigma <= 0.0:
            ...
            return (mu-incumbent-self.xi) * self._cdf((mu-incumbent-self.xi)/sigma) + sigma * self._pdf((mu-incumbent-self.xi)/sigma)
    ...
    @classmethod
    def read_from_config(cls, config: 'ConfigObj') -> 'ExpectedImprovement':
